...
class SOLITAIRE_ENGINE_API SWavFile : public SIAsset
{
...
class SOLITAIRE_ENGINE_API SWavLoader : public SIAssetLoader
{
public:
    /** Loads a WAV file from the specified file path into the provided output object. */
    virtual void Load(const SWString& FilePath, SIAsset* OutWAVFile) override;

    /** Unloads the Wav file, releasing any associated resources. */
    virtual void UnLoad(SIAsset* OutFile) override;

    /** Creates a new WAV file asset. */
    virtual SSharedPtr<SIAsset> CreateAsset() override;
